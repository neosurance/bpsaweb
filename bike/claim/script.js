...
function retriveClaimPayload() {
	var retVal = localStorage.getItem('bike_claimData_' + claimCtx.policy.proposalCode)
	if (retVal) {
		...
		if (getById('ter'))
			for (var j = retVal.photos.ter.length - 1; j >= 0; j--)
				usePhoto('ter', retVal.photos.ter[j])
	}
}

function storeClaimPayload() {
	localStorage.setItem('bike_claimData_' + claimCtx.policy.proposalCode, JSON.stringify(getClaimPayload()));
}
...
function getClaimPayload() {
	var payload = {
		...
		who : getById('who') ? getById('who').value.trim() : null,
...
function getPhotosPayload() {
	...
	if (getById('ter')) {
		var el = getById('ter').children
		for (var i = 0; i < el.length; i++)
			if (el[i].className == 'photo')
				retval.ter.push(el[i].children[0].src);
	}
	return retval
}
